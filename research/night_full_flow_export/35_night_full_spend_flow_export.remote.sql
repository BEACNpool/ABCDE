...
-- Data-only NIGHT full flow export.
-- Run from the ABCDE repo root so the relative NIGHT/data/*.csv paths resolve.
--
-- Graph model:
--   NIGHT UTxO node -> spending tx node -> child NIGHT UTxO node
--
-- This exports exact evidence. It does NOT apportion a parent UTxO's quantity
-- across children; any such allocation is a downstream visualization choice.
--
-- Uses TEMP tables only. No writes to public.*, explorer.*, or governance.*.
...
JOIN public.multi_asset ma ON ma.id = mto.ident
LEFT JOIN public.stake_address sa ON sa.id = txo.stake_address_id
LEFT JOIN public.tx_in ti
  ON ti.tx_out_id = txo.tx_id
...
WITH night_input AS (
  SELECT
    spend_tx_id_internal AS tx_id_internal,
    COUNT(*) AS night_input_utxo_count,
    SUM(qty_raw) AS night_input_qty_raw
  ...
  SELECT
    tx_id_internal,
    COUNT(*) AS night_output_utxo_count,
    SUM(qty_raw) AS night_output_qty_raw
  FROM night_utxo_nodes_all
...
WITH RECURSIVE reachable(utxo_row_id) AS (
  SELECT utxo_row_id
  FROM night_root_utxo

  UNION

  SELECT child.utxo_row_id
  FROM reachable r
  JOIN night_utxo_nodes_all parent
    ON parent.utxo_row_id = r.utxo_row_id
  JOIN night_utxo_nodes_all child
    ON child.tx_id_internal = parent.spend_tx_id_internal
  WHERE parent.spend_tx_id_internal IS NOT NULL
)
SELECT n.*
FROM night_utxo_nodes_all n
JOIN reachable r ON r.utxo_row_id = n.utxo_row_id;
...
JOIN (
  SELECT tx_id_internal FROM night_root_reachable_utxos
  UNION
  SELECT spend_tx_id_internal AS tx_id_internal
  FROM night_root_reachable_utxos
  WHERE spend_tx_id_internal IS NOT NULL
) r ON r.tx_id_internal = t.tx_id_internal;
...
LEFT JOIN night_root_reachable_utxos r
  ON r.utxo_row_id = a.utxo_row_id
...
UNION ALL SELECT 'root_utxo_qty_raw', COALESCE(SUM(qty_raw), 0)::text FROM night_root_utxo
...
UNION ALL SELECT 'reachable_current_leaf_qty_raw', COALESCE(SUM(qty_raw), 0)::text FROM night_current_leaves
...
UNION ALL SELECT 'unreachable_current_leaf_qty_raw', COALESCE(SUM(qty_raw), 0)::text FROM night_unreachable_current_leaves
...
\copy (SELECT * FROM night_mint_events ORDER BY block_no, tx_hash) TO './35_night_mint_events_abcde.csv' CSV HEADER
\copy (SELECT * FROM night_root_utxo ORDER BY block_no, tx_hash, tx_out_index) TO './36_night_root_utxo_abcde.csv' CSV HEADER
\copy (SELECT * FROM night_root_reachable_utxos ORDER BY block_no, tx_hash, tx_out_index) TO './37_night_reachable_utxo_nodes_abcde.csv' CSV HEADER
\copy (SELECT * FROM night_root_reachable_txs ORDER BY block_no, tx_hash) TO './38_night_reachable_tx_nodes_abcde.csv' CSV HEADER
\copy (SELECT * FROM night_edges_utxo_to_tx ORDER BY spend_block_no, target_tx_hash, source_utxo_node_id) TO './39_night_edges_utxo_to_tx_abcde.csv' CSV HEADER
\copy (SELECT * FROM night_edges_tx_to_utxo ORDER BY output_block_no, source_tx_hash, target_utxo_node_id) TO './40_night_edges_tx_to_utxo_abcde.csv' CSV HEADER
\copy (SELECT * FROM night_current_leaves ORDER BY qty_raw DESC, block_no, tx_hash, tx_out_index) TO './41_night_current_leaves_abcde.csv' CSV HEADER
\copy (SELECT * FROM night_unreachable_current_leaves ORDER BY qty_raw DESC, block_no, tx_hash, tx_out_index) TO './42_night_unreachable_current_leaves_abcde.csv' CSV HEADER
\copy (SELECT * FROM night_flow_summary) TO './43_night_flow_summary_abcde.csv' CSV HEADER

\echo Wrote NIGHT full flow CSV exports to NIGHT/data.
