...
Pool overlap re-run with correct tickers is **blocked on pg_hba.conf IP change** (client moved from prior IP to ${DB_HOST}). Once restored, re-run step 6 with EMURGO tickers.

---

## Blocking Issue — 2026-04-10

**pg_hba.conf IP change**: Client IP is now `${DB_HOST}`. The PostgreSQL server at `${DB_HOST}` does not allow connections from this IP.

Fix (on the db server):
```
# Add to pg_hba.conf:
host  cexplorer_replica  codex_audit  ${DB_HOST}/32  md5
# OR expand to subnet:
host  cexplorer_replica  codex_audit  ${DB_SUBNET}/24    md5
# Then reload:
