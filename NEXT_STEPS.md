...
**Status:** Active — DB blocked (client IP changed to 192.168.86.137; fix pg_hba.conf); new findings documented in `FULL_FINDINGS_2026-04-10_ADDENDUM.md`
...
- [x] **New client IP documented** — 192.168.86.137; pg_hba.conf update required on db server
...
```bash
psql -h 192.168.86.118 -p 5432 -U postgres -d cexplorer_replica \
...
**Pending DB fix** (client IP 192.168.86.137 must be added to pg_hba.conf):
```bash
export PGPASSWORD=faf26ee1042ce08c48a5a35b8eb26d03
