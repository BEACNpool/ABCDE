-- F11 receipt 2: external stake keys whose addresses fund cohort withdrawal
-- txs, ranked by how many of the 8 cohort keys they served.
...
select
  s2.view as funding_stake_address,
  count(distinct i.addr_id) as cohort_keys_served,
  count(distinct i.tx_id) as funded_withdrawal_txs,
  min(i.tx_id) as first_tx_id,
  max(i.tx_id) as last_tx_id
from inputs i
join public.stake_address s2 on s2.id = i.in_stake_id
where i.in_stake_id is distinct from i.addr_id
group by s2.view
order by cohort_keys_served desc, funded_withdrawal_txs desc
limit 25;
