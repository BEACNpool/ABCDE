// Prove the ANY-OF gate (piece 9, the PFP creator) against REAL mainnet state — build the
// real minting transaction from a real holder's UTxOs and have a node evaluate the Plutus
// script via Blockfrost /utils/txs/evaluate (ogmios EvaluateTx). No wallet, no signature,
// nothing submitted.
//
//   node evaluate9.mjs           # spend ONE piece only -> MUST evaluate
//                                #   (this is the discriminating case: the old all-of-eight
//                                #    validator rejects it, the any-of validator accepts it)
//   node evaluate9.mjs --none    # spend no pieces at all -> MUST fail as ScriptFailures
//
// The holder is auto-discovered from public Koios data (first current holder of a BYTTG
// asset) and kept in memory only — real addresses are never written to disk here.
import fs from 'fs';
...
const bp = JSON.parse(fs.readFileSync('applied-pfp9.json', 'utf8')).validators[0];
...
const SVG = fs.existsSync((process.env.PFP_EVAL_SVG || 'pfp-eval.svg'))
  ? fs.readFileSync((process.env.PFP_EVAL_SVG || 'pfp-eval.svg'))
  : Buffer.from('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 8 8"><rect width="8" height="8"/></svg>');
...
const kj = async p => (await fetch(KOIOS + p, { headers: { accept: 'application/json' } })).json();

// ── find a real holder of at least one piece, from public data ────────────────
const someAssets = await kj(`/policy_asset_list?_asset_policy=${REQUIRED[0]}`);
if (!someAssets.length) { console.error('no assets under piece 1?'); process.exit(1); }
let addrB = null;
for (const a of someAssets) {
  const hs = await kj(`/asset_addresses?_asset_policy=${REQUIRED[0]}&_asset_name=${a.asset_name}`);
  if (hs.length) { addrB = hs[0].payment_address; break; }
}
if (!addrB) { console.error('no holder found'); process.exit(1); }
...
console.log(`\n  wallet   ${addrB.slice(0, 20)}…   ${utxos.length} UTxOs   (holder of a BYTTG — never signed, never submitted)`);
...
const reqCount = u => policiesOf(u).filter(p => REQUIRED.includes(p)).length;

// clean = carries none of the eight (pure ada or unrelated tokens)
const clean = all.filter(u => reqCount(u) === 0)
  .sort((a, b) => Number(b.output().amount().coin().to_str()) - Number(a.output().amount().coin().to_str()));
if (!clean.length) { console.error('  holder has no piece-free UTxO to seed with'); process.exit(1); }

let spend;
if (NONE) {
  spend = clean.slice(0, 3);
} else {
  // the DISCRIMINATING carrier: a UTxO carrying as few of the eight as possible (ideally 1)
  const carriers = all.filter(u => reqCount(u) > 0).sort((a, b) => reqCount(a) - reqCount(b));
  if (!carriers.length) { console.error('  holder has no piece UTxO?!'); process.exit(1); }
  spend = [clean[0], carriers[0]];
  console.log(`  carrier  carries ${reqCount(carriers[0])} of the eight (want as few as possible)`);
}
...
meta[POLICY][nameStr] = { name: 'oligarCH PFP', mediaType: 'image/svg+xml', image: cut(uri),
...
const col = clean[clean.length - 1];
...
console.log(`  tx       ${(cbor.length / 2).toLocaleString()} B   asset ${nameStr}   policy ${POLICY.slice(0, 12)}…`);
console.log(`  mode     ${NONE ? 'NO pieces spent (MUST FAIL)' : 'ONE piece spent (must pass — the any-of case)'}`);
...
if (NONE) {
  console.log(okShape ? '\n  *** FAIL: zero pieces EVALUATED — the gate is not gating ***\n'
                      : '\n  PASS: zero pieces is rejected by the script\n');
  process.exit(okShape ? 1 : 0);
} else {
  console.log(okShape ? '\n  PASS: ONE piece satisfies the any-of gate on real chain state\n'
                      : '\n  *** FAIL: one piece did not evaluate ***\n');
